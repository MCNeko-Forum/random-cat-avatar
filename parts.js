...
window.PARTS = (() => {
  ...
  const OUTFITS = [
    ...
    { neck: true, bib: true, fn: () => `<path d="M256,333 L192,347 L184,466 Q256,488 328,466 L320,347 Z" fill="#f2d13d" stroke="${STROKE}" stroke-width="6" stroke-linejoin="round"/><circle cx="256" cy="408" r="9" fill="#e8a03c"/><circle cx="232" cy="428" r="7" fill="#e8a03c"/><circle cx="280" cy="428" r="7" fill="#e8a03c"/>` }, // 围兜（口水巾：顶部伸入脸后被脸盖住，围住脖子；bib 标记 = 超出身体宽度时由 avatar.js 等比缩小）
  ...
  const ITEMS = [
    ...
    (x, y) => `<g transform="translate(${x},${y})"><line x1="0" y1="34" x2="0" y2="-14" stroke="#a5795f" stroke-width="6" stroke-linecap="round"/><g stroke="${STROKE}" stroke-width="4" fill="#f28ba0" stroke-linejoin="round"><ellipse cx="-11" cy="-22" rx="7" ry="14" transform="rotate(-30 -11 -22)"/><ellipse cx="0" cy="-26" rx="7" ry="14"/><ellipse cx="11" cy="-22" rx="7" ry="14" transform="rotate(30 11 -22)"/></g></g>`, // 逗猫棒（竖直杆 + 左右对称羽毛扇：本体画正，外倾完全交给渲染层 ±15°）
